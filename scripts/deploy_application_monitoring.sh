...
if [[ $flagDestroy != "true" ]]; then
  ...
  terraform -chdir=../terraform/application init \
    ...
    -backend-config="key=${NEWRELIC_ACCOUNT_ID}-application-local"

  # Plan Terraform
  terraform -chdir=../terraform/application plan \
    -var NEW_RELIC_ACCOUNT_ID=$NEWRELIC_ACCOUNT_ID \
    -var NEW_RELIC_API_KEY=$NEWRELIC_API_KEY \
    -var NEW_RELIC_REGION=$NEWRELIC_REGION \
    -out "./tfplan"

  ## Add this for app relation dashboards
  # -var='app_relation_map=[{"app_name": "MAIN_APP_NAME","related_apps": [{"account_id": ACCOUNT_ID,"app_names": "RELATED_APP_NAME_1,RELATED_APP_NAME_2"},{"account_id": ACCOUNT_ID,"app_names": "RELATED_APP_NAME_1,RELATED_APP_NAME_2"}]}]' \

  # Apply Terraform
  if [[ $flagDryRun != "true" ]]; then
    terraform -chdir=../terraform/application apply tfplan
  fi
else
